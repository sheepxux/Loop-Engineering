...
- **Orphaned state-lock acquisition gate**: a path ending in `.lock.acquire` means a process may have crashed during the short acquire/reclaim doorway. SuperLoop deliberately does not auto-delete this gate. Stop every process using the loop, inspect its `owner.json`, confirm that owner is dead, back up the loop, and only then remove the gate; rerun `doctor`, validation, and a dry-run before resuming.
