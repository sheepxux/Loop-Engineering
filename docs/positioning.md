...
**Loops are overrated where topology is underrated.** Feedback-dense, single-goal tasks — CI triage, dependency updates, QA passes — are well served by a simple loop, and that is the 80% use case v1 targets. Genuinely hard work is a graph of deeply coupled subtasks where agents get lost three levels deep. Protocol v1 deliberately models a single-lane loop: discover N bounded items, process them one at a time. It does not model dependency graphs between items, and it should not pretend to. If that expressiveness comes later, it is a protocol extension, not a rendering trick.
...
- Current (`v1.0.x`): complete canonical Skill, protocol v1, evidence-bound evaluator artifacts, digest-bound strategy approval and rollback, and a local Runner with leases, interval scheduling, durable events, and opt-in command execution.
